...
//solidity中不同的数据结构，使用mapping映射来完成需求
...
contract HelloWorld{
    ...
    //因为这里换成使用mapping方法就不需要使用数组了
    //Info[] infos; //声明一个Info结构体的array数组起名infos

    mapping(uint256 key => Info info) infoMapping;
    //这里括号内的是mapping的对应关系，储存两个值
    //一个是uint256类型的key（也就是id）取名为key
    //还有就是Info这个结构体对象作为value，取名为info
    //给mapping再取一个名字叫infoMapping
    ...
    function setHelloWorld(string memory newString, uint256 _Id) public {
        ...
        //因为我们想要从id来找到这个键值对，所以方括号中要输入命名为key的id
        //这样我们就把结构体对象压入mapping中了
    ...
    //用户需要告诉我们他想要调用哪个id的结构体，所以这个say函数需要一个传入参数_id
        ...
        if(infoMapping[_id].addr == address(0x0)) {
            //至于如何验证这个id是否存在，首先我们找到结构体中那个属性易于判断为空
            //我们找到地址addr变量，如果id不存在，地址就会为空
            //address(0x0)表示空地址，如果为空，返回默认值
